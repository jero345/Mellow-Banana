...
React 19 + Vite 8 + Tailwind CSS 4 + React Router 7. Sin dependencias de UI:
todo el sistema visual sale del PDF (color, tipografía, retículas, gestos).
...
- **Curva de animación**: `ease-brand` (`cubic-bezier(.22,1,.36,1)`).
...
- **`ReelModal`** — el orbe “Play Reel” del hero abre `public/media/reel.mp4`.
...
1. **El reel pesa 100 MB** (`public/media/reel.mp4`, copiado de `REEL25.mp4`).
   Funciona, pero antes de publicar conviene comprimirlo (H.264 ~1080p) o
   servirlo desde Mux / Cloudflare Stream y cambiar el `src` de `ReelModal`.
2. **El formulario abre un borrador de correo** (`mailto:`) porque no hay backend.
   En `ContactForm.jsx`, reemplaza `submit` por un `fetch` a su endpoint
   (Formspree, Resend, webhook de n8n) cuando exista.
3. **El panel “Anterior”** de la comparación tipográfica está **vacío en el PDF**,
   así que se publica solo el panel “Nuevo”. Cuando tengan el arte, agréguenlo
   como segundo item y cambien ese bloque a `kind: 'duo'`.
4. **Casos de estudio**: solo Federación viene completo en el PDF. Los otros siete
   proyectos tienen portada e intro y esperan sus `blocks`.
5. **Sección 05 “El Equipo”**: el PDF solo trae la foto del estudio. Si quieren
   retratos individuales, es una grilla más en `About.jsx`.
6. **Redes**: las URLs en `site.js` son las probables — confírmenlas.
...
- Menú móvil abre, navega y libera el scroll; switch Es/En cambia copy y
  ...
  carrusel responden.
- Build de producción sirve rutas profundas correctamente.
